...
function Joystick:Ctor(gameObject, stickObj, bgObj, startDragCall, dragingCall, endDragCall)
    ...
    self.pointerHandler:AddCall(PointerHandler.DOWN, handler(self, self.DragStart))
    self.pointerHandler:AddCall(PointerHandler.UP, handler(self, self.EndDrag))
...
function Joystick:DragStart(data)
    ...
    --end
    self.dragingHandler = AddEventListener(Stage, Event.UPDATE, handler(self, self.Draging))
...
function Joystick:EndDrag()
    ...
    RemoveEventListener(Stage, Event.UPDATE, self.dragingHandler)
